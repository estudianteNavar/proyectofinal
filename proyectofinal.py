...
def abrir_reportes():
            ...
            for linea in archivo:
                if linea.strip():
                    ...
                    if len(datos) == 4:
                        ...
                        # Acumular datos
                        total_ingresos += float(datos[3])
                        total_cantidad += int(datos[2])
    except FileNotFoundError:
    ...
    frame_totales.pack(pady=20)

    lbl_total_cant = tk.Label(
        frame_totales,
        text=f"Total de artículos vendidos: {total_cantidad}",
        font=("Arial", 14, "bold"),
        bg="#f2f2f2"
    )
    lbl_total_cant.pack(pady=5)
    ...
    lbl_total_ingresos.pack(pady=5)
