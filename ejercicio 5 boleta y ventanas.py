...
ventana_principal.title("ejercico 5")
ventana_principal.geometry("300x200")

def ventana_autor():
    ventana_autor = Toplevel(ventana_principal)
    ventana_autor.title("Autor del proyecto")
    ventana_autor.geometry("300x200")

    etiqueta = tk.Label(ventana_autor, text="creado por: Navara Galindo", font=("Arial", 12))
    etiqueta.pack(pady=10)
    
    boton_cerrar = tk.Button(ventana_autor,text="cerrar", command= ventana_autor.destroy)
    boton_cerrar.pack(padx=10)
 
def ventana_boleta():
   
    ventana_boleta = Toplevel(ventana_principal)
    ventana_boleta.title("boleta calificaciones")
    

    #nombre alumno 
    tk.Label(ventana_boleta, text="Samntha").grid(row=0, column=0, sticky="w")
    entry_nombre = tk.Entry(ventana_boleta,width=40)
    entry_nombre.grid(row=0, column=1, columnspan=4, sticky="we")

    #encabezados de la tabla 
    encabezados = ["Materia", "Unidiad", "Unidad2", "Unidad3", "promedio"]
